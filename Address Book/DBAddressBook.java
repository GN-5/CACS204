...
public class DBAddressBook implements ActionListener {
    // frame details
    ...
    private String dbURL = "jdbc:mysql://localhost:3306/addressdb";
    ...
    DBAddressBook() {
        ...
        // activating connection instance
    ...
    private void addIntoDB() {
        String sql = "INSERT INTO contacts(name, age, address, phone, gender, dob) VALUES (?, ?, ?, ?, ?, ?)";

        try (PreparedStatement statement = con.prepareStatement(sql)) {
            ;
            statement.setString(1, address.getName());
            statement.setString(2, address.getAge());
            statement.setString(3, address.getAddress());
            statement.setString(4, address.getPhone());
            statement.setString(5, address.getGender());
            statement.setString(6, address.getDate());

            int rows = statement.executeUpdate();
            if (rows > 0) {
                JOptionPane.showMessageDialog(frmMain, "Rows inserted successfully", "Success",
                        JOptionPane.DEFAULT_OPTION);
            }
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(frmMain, "SQL Error", "Failure", JOptionPane.ERROR_MESSAGE);
        }
